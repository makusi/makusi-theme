...
/**
*  Template Name: Page Enable Donation
* 
*/
            ...
            <?php else : ?>
                ...
                <form method="post" action="<?php the_permalink(); ?>" enctype="multipart/form-data">
                    <table>
                        <tr>
                            <th>
                                <label><?php echo __('Entidad','makusi'); ?></label><br />
                                (4 <?php echo __('caracteres','makusi'); ?>)
                            </th>
                            <th>
                                <label><?php echo __('Oficina','makusi'); ?></label><br />
                                (4 <?php echo __('caracteres','makusi'); ?>)
                            </th>
                            <th>
                                <label><?php echo __('Control','makusi'); ?></label><br />
                                (2 <?php echo __('caracteres','makusi'); ?>)
                            </th>
                            <th>
                                <label><?php echo __('Número de Cuenta','makusi'); ?></label><br />
                                (10 <?php echo __('caracteres','makusi'); ?>)
                            </th>
                        </tr>
                        <tr>
                            <td>
                                <input type="text" name="bank_entity" <?php if($bank_entity_missing == TRUE){ ?>class="field_missing"<?php } ?> value="<?php echo $_POST['bank_entity']; ?>" size="4" />
                            </td>
                            <td>
                                <input type="text" name="bank_office" <?php if($bank_office_missing == TRUE){ ?>class="field_missing"<?php } ?> value="<?php echo $_POST['bank_office']; ?>" size="4" />
                            </td>
                            <td>
                                <input type="text" name="bank_control" <?php if($bank_control_missing == TRUE){ ?>class="field_missing"<?php } ?> value="<?php echo $_POST['bank_control']; ?>" size="2" />
                            </td>
                            <td>
                                <input type="text" name="bank_account" <?php if($bank_account_missing == TRUE){ ?>class="field_missing"<?php } ?> value="<?php echo $_POST['bank_account']; ?>" size="10" />
                            </td>
                        </tr>
                    </table>
                    <br />
                    <input type="checkbox" name="activate_international" id="activate_international" /> <?php echo __('Internacional','makusi'); ?>
